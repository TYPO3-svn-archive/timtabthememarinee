...
***************************************************************/
/**
 * class.tx_timtabthememarinee_fe.php
 *
...
class tx_timtabthememarinee_fe extends tslib_pibase {
	var $cObj; // The backReference to the mother cObj object set at call time
	// Default plugin variables:
	var $prefixId 		= 'tx_timtabthememarinee_fe';		// Same as class name
	var $scriptRelPath 	= 'class.tx_timtabthememarinee_fe.php';	// Path to this script relative to the extension dir.
	var $extKey 		= 'timtab_theme_marinee';	// The extension key.
	...
	function substituteMarkers() {
		if($this->calledBy == 'tt_news') {	
			$this->markerArray['###MARINEE_POST_ID###'] = $this->conf['data']['uid'];
			$this->markerArray['###MARINEE_IN###'] = $this->pi_getLL('posted_in');
			$this->markerArray['###MARINEE_BY_ON###'] = sprintf($this->pi_getLL('by_on'), $this->conf['data']['author']);
			$this->markerArray['###MARINEE_COMMENT_LINK###'] = $this->getCommentLink($this->conf['data']);		
		} elseif($this->calledBy == 've_guestbook') {		
			$this->markerArray['###MARINEE_SAID###'] = $this->pi_getLL('said');
		}
	}
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/timtab_theme_marinee/class.tx_timtabthememarinee_fe.php']) {
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/timtab_theme_marinee/class.tx_timtabthememarinee_fe.php']);
}
